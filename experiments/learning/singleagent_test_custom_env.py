...
# It is the same as singleagent_test.py but with addition support for DroneReach environment
import os
import time
...
EPISODE_REWARD_THRESHOLD = 20000
# -0 # Upperbound: rewards are always negative, but non-zero
...
if __name__ == "__main__":
    ...
    parser.add_argument('--env',        default='reach',      type=str,             choices=['takeoff', 'hover', 'flythrugate', 'tune'], help='Task (default: hover)', metavar='')
    ...
    if env_name == "reach-aviary-v0":
        ...
        train_env = make_vec_env(DroneReach,
                                    env_kwargs=sa_env_kwargs,
                                    n_envs=ARGS.cpu,
                                    seed=0)
    ...
    if ARGS.obs == ObservationType.KIN: 
        ...
        if env_name == "reach-aviary-v0":
            eval_env = make_vec_env(DroneReach,
                                        env_kwargs=sa_env_kwargs,
                                        n_envs=ARGS.cpu,
                                        seed=0)
    ...
    wandb.init(
        ...
        notes="drone learn.py -> gym-pybullet-drones",
    )

    experiment_name = "-Drone-learnpy"
    wandb.run.name = wandb.run.name + experiment_name + "-" + "Drone_learnpy"
    wandb.run.save()
    ...
    model.learn(total_timesteps=800000, #int(1e12),
                callback=[eval_callback, WandbCallback(), checkpoint_callback],
                log_interval=100,
                )
