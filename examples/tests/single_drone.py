...
if __name__ == "__main__":
    ...
    H_STEP = .05
    R = .3
    # Initialization the configuration for the drones
    INIT_XYZS = np.array([[0, 0,  H+H_STEP]])
    INIT_RPYS = np.array([[0, 0, 0]])
    # Physics step -> Simulation frequency and the control frequency
    ...
    for i in range(NUM_WP):
        TARGET_POS[i, :] = R*np.cos((i/NUM_WP)*(2*np.pi)+np.pi/2)+INIT_XYZS[0, 0], R*np.sin((i/NUM_WP)*(2*np.pi)+np.pi/2)-R+INIT_XYZS[0, 1], INIT_XYZS[0, 2] #i/10
    wp_counters = np.array([int((NUM_WP/6)%NUM_WP)])
